...
void modulateAskByte(bool modulated_ASK_bits[], unsigned char data_byte) {
    ...
    uint16_t byte = 0; // the 11-bit byte containing all the necessary bits + 8 data bits
    /* start bit */
    modulated_ASK_bits[0] = 0;

    /* data bits */
    for (int i = 0; i < 8; i++) {
        modulated_ASK_bits[i+1] = (data_byte >> i) & 0x01; // shift bits and store LSB first
        if (modulated_ASK_bits[i+1]) {
            num_ones++; // increment number of ones if bit is set
        }
    }

    /* parity bit */
    modulated_ASK_bits[9] = (num_ones % 2 == 0); // calculate parity bit

    /* stop bit */
    modulated_ASK_bits[10] = 1;

    for (int i = 0; i < 11; i++) {
        byte |= (modulated_ASK_bits[i] ? 1 : 0) << (10 - i);
    }
}
...
uint8_t demodulateFskByte(uint16_t encoded_byte,DecodedDataPacket *packet, uint8_t position){
	...
	if (check_sum!=data_byte){
		//return 4 ;//check_sum; //VYRESIT JINDY				
	}*/
	check_sum+=packet->header;
	check_sum+=packet->message[0];
	if (check_sum!=data_byte){
	  return 4;
	}
	} else{
